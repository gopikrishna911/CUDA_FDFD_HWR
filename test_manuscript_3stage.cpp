...
/*--------------------------------------------------------------------------
 * GPU memory used (MB), via cudaMemGetInfo
 *------------------------------------------------------------------------*/
static double gpu_used_mb(void) {
    size_t f = 0, t = 0;
    cudaMemGetInfo(&f, &t);
    return (double)(t - f) / (1024.0 * 1024.0);
}

/*--------------------------------------------------------------------------
 * Memory footprint profiler. Mirrors the real allocation sequence of the
 * IBC complex eigensolve and measures the cudaMemGetInfo() delta at each
 * key allocation point: PEC field arrays, conformal IBC operator (complex
 * field arrays + conformal geometry), solution vectors, and the GMRES(50)
 * Arnoldi basis. Reports a per-category table and the peak total.
 * No eigensolve is run — this is fast.
 *------------------------------------------------------------------------*/
static void profile_stage_memory(const StageConfig* sc)
{
    printf("\n================================================================\n");
    printf("  GPU MEMORY FOOTPRINT: %s\n", sc->label);
    printf("================================================================\n");

    /* --- geometry / grid (same as run_stage) --- */
    PipeConfig pipes;
    ...
    /* z-buffer policy (see run_stage below for the full rationale): even
     * when no endcap pipes are configured we add a small PEC buffer in z
     * so the endplates are INTERIOR surfaces, not grid edges. This makes
     * k_endplate_z0/zL >= 0 and the conformal-pipe operator's grid-plane
     * IBC kernels fire — without it, endplate copper loss is missing and
     * Q(eig) is inflated by ~1.8x. */
    static const int NZ_BUFFER_NO_ENDCAP = 4;   /* ~66 mm at dz ~16.6 mm */
    double pipe_len_r = sc->pipe_length;
    int Nr_pipe_eff = sc->Nr_pipe;
    double dz_cav = sc->L / sc->Nz_cavity;
    int Nz_pipe_z0 = sc->use_endcap_pipes ? (int)ceil(endcap.z0_extension / dz_cav)
        : NZ_BUFFER_NO_ENDCAP;
    int Nz_pipe_zL = sc->use_endcap_pipes ? (int)ceil(endcap.zL_extension / dz_cav)
        : NZ_BUFFER_NO_ENDCAP;
    double pipe_len_z0 = Nz_pipe_z0 * dz_cav;
    double pipe_len_zL = Nz_pipe_zL * dz_cav;
    ...
    int k_z0 = (Nz_pipe_z0 > 0) ? Nz_pipe_z0 : -1;
    int k_zL = (Nz_pipe_zL > 0) ? Nz_pipe_z0 + sc->Nz_cavity : -1;
    double z0_offset = (k_z0 >= 0) ? k_z0 * grid.dz : 0.0;
    ...
    long long dofs = (long long)grid.Nr * grid.Nphi * grid.Nz * 3;
    printf("  Grid: Nr=%d, Nphi=%d, Nz=%d   (%.2f M complex DOFs)\n",
        grid.Nr, grid.Nphi, grid.Nz, dofs / 1e6);

    /* ===== Baseline AFTER grid metadata is on the device ===== */
    double m_base = gpu_used_mb();

    /* ===== 1. PEC operator: real field working arrays ===== */
    GPU_PipeOperator gpu_pec;
    gpu_pipe_operator_init(&gpu_pec, &cpu_op, &mask);
    gpu_pipe_operator_set_endplates(&gpu_pec, k_z0, k_zL, &endcap, &grid, z0_offset);
    double m_pec = gpu_used_mb();

    /* ===== 2. Conformal IBC operator: complex field arrays + geometry ===== */
    GPU_ConformalPipeOperator gpu_cfm;
    gpu_conformal_pipe_operator_init(&gpu_cfm, &cpu_op, &mask, &cd,
        k_z0, k_zL, &endcap, &grid, z0_offset);
    double m_cfm = gpu_used_mb();

    /* ===== 3. Solution vectors (real seed + complex eigenvector) ===== */
    double* d_x;    gpu_vector_alloc(&d_x, n);
    double* d_x_cx; gpu_vector_alloc(&d_x_cx, n2);
    double m_vec = gpu_used_mb();

    /* ===== 4. GMRES(50) Arnoldi basis + work (same as the real solve) ===== */
    ComplexEigensolverWorkspace cws;
    complex_eigensolver_workspace_init(&cws, n, 50);
    double m_gmres = gpu_used_mb();

    /* ===== Report ===== */
    double one_cx_vec_mb = (double)n2 * sizeof(double) / (1024.0 * 1024.0);
    printf("\n  One complex vector (2*n DOFs, fp64): %.1f MB\n", one_cx_vec_mb);
    printf("\n  %-44s %12s %12s\n", "Category", "Size [MB]", "Cumul [MB]");
    printf("  -----------------------------------------------------------------------\n");
    printf("  %-44s %12.1f %12.1f\n", "PEC operator (real field arrays + masks)",
        m_pec - m_base, m_pec - m_base);
    printf("  %-44s %12.1f %12.1f\n", "Conformal IBC operator (cx arrays + geom)",
        m_cfm - m_pec, m_cfm - m_base);
    printf("  %-44s %12.1f %12.1f\n", "Solution vectors (real + complex)",
        m_vec - m_cfm, m_vec - m_base);
    printf("  %-44s %12.1f %12.1f\n", "GMRES(50) Arnoldi basis + workspace",
        m_gmres - m_vec, m_gmres - m_base);
    printf("  -----------------------------------------------------------------------\n");
    printf("  %-44s %12.1f %12.1f\n", "PEAK TOTAL (above baseline)",
        m_gmres - m_base, m_gmres - m_base);
    printf("\n  Peak resident on device (incl. driver/context): %.2f GB\n",
        m_gmres / 1024.0);
    printf("  (one A5000 holds %.1f GB; headroom %.1f GB)\n",
        24284.0 / 1024.0, (24284.0 - m_gmres) / 1024.0);

    /* cleanup */
    complex_eigensolver_workspace_free(&cws);
...
static StageResult run_stage(const StageConfig* sc)
{
    ...
    /* --- grid (z-buffer policy) ---
     *
     * BUG FIX (vs. the previous version of this file): when use_endcap_pipes=0
     * we were setting Nz_pipe_z0 = Nz_pipe_zL = 0, putting the endplates at
     * grid edges k=0 and k=Nz. That makes k_endplate_z0/zL = -1, and the
     * conformal-pipe operator's grid-plane endplate IBC kernels are guarded
     * by `if (k_endplate_z0 >= 0)` in cuda_conformal_pipe.cu — so they do
     * NOT fire. The IBC operator then has no copper-loss term for the
     * endplates, even though the surface-integral Q post-processing does
     * see them. That's why this file used to print Q(eig) ~86000 vs
     * Q(surf) ~48000 for stages 1 and 2: ~44% of the loss (the endplates)
     * was missing from the eigenvalue.
     *
     * Fix: same trick already used in the radial direction (see the existing
     * Nr_pipe buffer comment below) — always pad z with a small PEC buffer
     * so the endplates are INTERIOR surfaces with k_endplate_z0/zL >= 0.
     * The empty endcap config gracefully degrades to "full PEC endplate"
     * in both material_mask_build_full() and gpu_pipe_operator_set_endplates(),
     * so the buffer cells stay solid PEC and the physics is unchanged — only
     * the IBC operator picks up its missing endplate-loss term.
     */
    static const int NZ_BUFFER_NO_ENDCAP = 4;   /* ~66 mm at dz ~16.6 mm */
    ...
    double z0_offset = (k_endplate_z0 >= 0) ? k_endplate_z0 * grid.dz : 0.0;
...
int main(int argc, char** argv)
{
    ...
    /* Memory-profile mode: ./test_manuscript_3stage memprofile
     * Profiles the production grid for the unperturbed case (Nz=84) and the
     * full model (Nz=117, via endcap pipes) and exits — no eigensolve. */
    if (argc > 1 && strcmp(argv[1], "memprofile") == 0) {
        StageConfig mu = base;
        mu.label = "Unperturbed (Nr=88, Nphi=256, Nz=84)";
        mu.use_radial_pipes = 0; mu.use_endcap_pipes = 0; mu.use_inner_ports = 0;
        StageConfig mf = base;
        mf.label = "Full model (Nr=88, Nphi=256, Nz=117)";
        mf.use_radial_pipes = 1; mf.use_endcap_pipes = 1; mf.use_inner_ports = 1;
        profile_stage_memory(&mu);
        profile_stage_memory(&mf);
        printf("\n================================================================\n");
        printf("  MEMORY PROFILE DONE.\n");
        printf("================================================================\n");
        return 0;
    }
