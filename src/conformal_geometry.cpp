...
void conformal_data_build(
    ...
    for (int p = 0; p < radial_pipes->num_pipes; p++) {
         ...
         *
         * Pipes are radial — axis along r-hat. The pipe wall is a cylinder
         * around that axis. The local tangent vectors at the wall are:
         *   t1 = r-hat (along pipe axis)         → E_r is tangential
         *   t2 = circumferential around pipe     → mix of E_φ and E_z
         *
         * So all three E components have tangential components at the wall:
         *   - E_r:  always tangential (along pipe axis)
         *   - E_φ:  tangential where φ-hat is along t2
         *   - E_z:  tangential where z-hat is along t2
         *
         * IBC weight = wall_surface_element_in_dual / V_dual.
         * For each E-edge, surface element ≈ arc_len_of_wall_in_(φ,z)_dual × dr.
         *
         * Er edges are PARALLEL to the wall (along the axis), so they are
         * never cut (edge_frac is 0 or 1). The trigger here is "wall passes
         * through Er's dual cell", i.e. arc_len > 0 — NOT frac < 1.
         *-----------------------------------------------------------------*/

         /* Er at (i+½, j, k): IBC weight.
          * Dual face is (φ,z) rectangle at r = a + (i+½)dr,
          * spanning φ ∈ [(j-½)dφ, (j+½)dφ], z ∈ [(k-½)dz, (k+½)dz]. */
        for (int i = dmax(i_lo, 0); i < dmin(i_hi, Nr); i++) {
            double r_er = grid->a + (i + 0.5) * dr;
            if (r_er <= r_outer) continue;
            if (r_er >= r_outer + pipe_len - 1e-10) continue;
            ...
            for (int dj = -dj_max; dj <= dj_max; dj++) {
                ...
                for (int dk = -dk_max; dk <= dk_max; dk++) {
                    ...
                    double arc_len = wall_arc_length_Hr(
                        r_er, phi_lo, phi_hi, z_lo, z_hi,
                        phi_c, z_c, b, R_aper, R_pipe, pipe_len);

                    if (arc_len > 0.0) {
                        /* V_dual = dual_area_Er · dr, where dual_area_Er is
                         * the (φ,z) face area at r_er (cut to vacuum portion). */
                        double V_dual = cd->dual_area_Er[idx] * dr;
                        if (V_dual > 1e-30) {
                            double w = arc_len * dr / V_dual;
                            cd->ibc_weight_Er[idx] = dmax(cd->ibc_weight_Er[idx], w);
                        }
                    }
                }
            }
        ...
        /* Eφ and Ez: tangential at integer-i positions. */
        for (int i = dmax(i_lo, 0); i <= dmin(i_hi, Nr); i++) {
            double r = grid->a + i * dr;
