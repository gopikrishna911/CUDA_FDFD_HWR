...
int main(void) {
    ...
    for (int pass = 0; pass < num_passes; pass++) {
        ...
        port.radius = aperture_radius; port.pos2 = L / 2.0 + endcap_pipes.z0_extension;
    ...
    /* z0_offset MUST be snapped to the actual grid (Nz_pipe_z0 * grid.dz),
     * not the requested endcap.z0_extension. grid_init_with_all_pipes()
     * computes Nz_pipe_z0 = ceil(extension/dz), so the actual physical
     * offset to the grid origin is Nz_pipe_z0 * grid.dz. Using the
     * unsnapped requested value puts every radial pipe at the wrong
     * k_cen (shifted by one cell), giving a different material mask and
     * a wrong Q (~54000 instead of ~48126). */
    double z0_offset = Nz_pipe_z0 * grid.dz;
    ...
    /* IBC unmask fixup: surface cells (outer wall, endplates, pipe walls) need
     * edge_frac=1 so the conformal-stencil IBC face-area weighting is correct.
     * Without this step, the conformal edge fractions at IBC surface cells
     * remain at their geometric vacuum-fraction values and the operator
     * under-counts wall loss -> Q comes out ~30% high.
     * (This block exists in test_conformal_ibc.cpp; it was missing here.) */
    {
...

    {
        ...
        gpu_conformal_pipe_operator_init(&gpu_cfm, &cpu_op, &mask, &cd, k_endplate_z0, k_endplate_zL, &endcap_pipes, &grid, z0_offset);
        ...
        {
            /* Pass both real and imaginary parts: the exporter will phase-rotate
             * so Re(v) becomes the physical TEM-like mode before writing.        */
            FieldMapExportConfig fmcfg;
            memset(&fmcfg, 0, sizeof(fmcfg));
            fmcfg.k_cavity_start = k_endplate_z0;
            fmcfg.k_cavity_end = k_endplate_zL;
            fmcfg.z0_offset = z0_offset;
            fmcfg.omega = 2.0 * M_PI * ibc_result.frequency_Hz;
            fmcfg.normalize_to_peak = 1;
            fmcfg.flip_sign_for_positive_Er = 1;
            fmcfg.eigenvector_imag = h_x_cx + n;   /* imag block follows real */

            export_cavity_field_map(&cpu_op, h_x_cx, &fmcfg, "cavity_field.bin");
        }
