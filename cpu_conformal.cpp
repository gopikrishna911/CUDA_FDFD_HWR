...
/* Wall-clock seconds for progress/ETA reporting (monotonic where available). */
static double conf_wall_seconds(void) {
    ...
    const GridParams* grid)
{
    ...
    /* Endplate aperture masks from the endcap config (B2): 1 = endcap pipe
     * vacuum hole through the endplate -> skip endplate IBC there. Mirrors
     * cuda_conformal_pipe.cu's endcap-mask block exactly, including the
     * z-probe convention (-0.001 below z0, L_phys+0.001 above zL). With a
     * NULL or empty endcap config, masks stay NULL => solid endplates. */
    if (endcap_pipes && endcap_pipes->num_pipes > 0 &&
        (k_z0 >= 0 || k_zL >= 0)) {
        int Nr = grid->Nr, Nphi = grid->Nphi, Nr1 = Nr + 1;
        double L_phys = (k_zL - k_z0) * grid->dz;
        int er_size = Nr * Nphi, ephi_size = Nr1 * Nphi;

        if (k_z0 >= 0) {
            cop->mask_Er_z0   = (int*)calloc(er_size,   sizeof(int));
            cop->mask_Ephi_z0 = (int*)calloc(ephi_size, sizeof(int));
            for (int j = 0; j < Nphi; j++) {
                double phi = (j + 0.5) * grid->dphi;
                for (int i = 0; i < Nr; i++) {
                    double r = grid->a + (i + 0.5) * grid->dr;
                    if (point_is_vacuum_endcap(endcap_pipes, L_phys, r, phi, -0.001))
                        cop->mask_Er_z0[i + Nr * j] = 1;
                }
                for (int i = 0; i <= Nr; i++) {
                    double r = grid->a + i * grid->dr;
                    if (point_is_vacuum_endcap(endcap_pipes, L_phys, r, phi, -0.001))
                        cop->mask_Ephi_z0[i + Nr1 * j] = 1;
                }
            }
            int c1 = 0, c2 = 0;
            for (int i = 0; i < er_size; i++)   c1 += cop->mask_Er_z0[i];
            for (int i = 0; i < ephi_size; i++) c2 += cop->mask_Ephi_z0[i];
            printf("  Endplate z0 masks (CPU): Er=%d Ephi=%d pipe-hole cells\n", c1, c2);
        }
        if (k_zL >= 0) {
            cop->mask_Er_zL   = (int*)calloc(er_size,   sizeof(int));
            cop->mask_Ephi_zL = (int*)calloc(ephi_size, sizeof(int));
            for (int j = 0; j < Nphi; j++) {
                double phi = (j + 0.5) * grid->dphi;
                for (int i = 0; i < Nr; i++) {
                    double r = grid->a + (i + 0.5) * grid->dr;
                    if (point_is_vacuum_endcap(endcap_pipes, L_phys, r, phi, L_phys + 0.001))
                        cop->mask_Er_zL[i + Nr * j] = 1;
                }
                for (int i = 0; i <= Nr; i++) {
                    double r = grid->a + i * grid->dr;
                    if (point_is_vacuum_endcap(endcap_pipes, L_phys, r, phi, L_phys + 0.001))
                        cop->mask_Ephi_zL[i + Nr1 * j] = 1;
                }
            }
            int c1 = 0, c2 = 0;
            for (int i = 0; i < er_size; i++)   c1 += cop->mask_Er_zL[i];
            for (int i = 0; i < ephi_size; i++) c2 += cop->mask_Ephi_zL[i];
            printf("  Endplate zL masks (CPU): Er=%d Ephi=%d pipe-hole cells\n", c1, c2);
        }
    }
    ...
    int max_iter, double tol, int gmres_restart)
{
    ...
    /* Option A: proper (shift-updating) RQI. We seed the shift at the PEC k2
     * and then UPDATE it to the current Rayleigh quotient each iteration
     * (see end of loop). This gives the fast RQI convergence and keeps the
     * inner GMRES system (A - sigma I) well-targeted at high resolution,
     * where a FIXED shift stalls. We keep the shift REAL for the GMRES solve
     * (k2_im ~ 1e-4 is negligible for conditioning the inner solve; the loss
     * still enters fully through the IBC alpha and the Rayleigh quotient). */
    sigma_re = sigma_init;
    sigma_im = 0.0;

    printf("\n  Complex RQI (conformal pipe, CPU, threads=%d, shift-updating):\n",
           cpu_omp_num_threads());
    printf("  %-5s %-15s %-12s %-12s %-12s %-6s %-8s %-9s\n",
           "Iter", "k2_re", "k2_im", "Q", "Residual", "GMRES", "iter_s", "ETA");
    printf("  --------------------------------------------------"
           "------------------------------------\n");

    double sigma_re_old = sigma_re, sigma_im_old = sigma_im;
    double rqi_t0 = conf_wall_seconds();   /* whole-solve start */

    for (int iter = 0; iter < max_iter; iter++) {
        double iter_t0 = conf_wall_seconds();
        alpha = cpu_conf_alpha(sigma_re, conductivity);
        /* First iteration uses a loose, capped solve to clear gross error;
         * thereafter solve tighter. With shift updating, the inner system is
         * well-targeted so these converge in far fewer GMRES iters. */
        int    gmres_maxiter = (iter == 0) ? 300  : 2000;
        double gmres_tol     = (iter == 0) ? 1e-1 : 1e-4;
        ...
        double iter_s = conf_wall_seconds() - iter_t0;
        /* ETA: average iteration time so far x remaining iterations. The RQI
         * usually converges before max_iter, so this is an UPPER bound that
         * shrinks as the residual drops; useful for spotting a runaway solve. */
        double avg_s  = (conf_wall_seconds() - rqi_t0) / (double)(iter + 1);
        double eta_s  = avg_s * (double)(max_iter - (iter + 1));

        printf("  %-5d %15.10f %12.5e %12.1f %12.5e %6d %7.2fs %7.0fs\n",
               iter, sigma_re_new, sigma_im_new, Q, res, ls.iterations,
               iter_s, eta_s);
        fflush(stdout);   /* so progress shows live during a long run */
        ...
        if (res < tol || (rel < tol && iter > 0)) {
            r.converged = 1;
            printf("  Converged!  f = %.6f MHz, Q = %.1f  (RQI wall %.1fs)\n",
                   freq / 1.0e6, Q, conf_wall_seconds() - rqi_t0);
            break;
        }
        sigma_re_old = sigma_re_new;
        sigma_im_old = sigma_im_new;
        /* Option A: UPDATE the shift to the current Rayleigh quotient so the
         * inner system (A - sigma I) tracks the eigenvalue (fast RQI). Shift
         * kept real for inner-solve conditioning; loss enters via alpha + RQ.
         * Residual-gated: only move the shift once residual < 1.0, so the
         * early inflated Rayleigh quotient (from interior IBC boundary
         * weights) cannot corrupt it. MUST match cuda_eigensolver.cu. */
        if (sigma_re_new > 0.0 && res < 1.0) sigma_re = sigma_re_new;
        sigma_im = 0.0;
    }
